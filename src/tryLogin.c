...
void tryLogin(char arqFisicoRoot[], char arqFisicoClientes[], char arqFisicoLogin[], char arqFisicoImobiliarias[]){
   ...
   if (strcmp(user, root.username) == 0 && strcmp(senha, root.passwd) == 0){
       //loga como root
       logAsRoot(arqFisicoClientes, arqFisicoLogin, arqFisicoImobiliarias);
   }
   //Tentative de log pelo usuário root, mas senha incorreta
	   ...
	   arquivoVazio = 0;//se entrou no laço, então arquivo não está vazio
	      ...
	      //loga como cliente
              logAsNormalUser(arqFisicoClientes, arqFisicoImobiliarias, userLog);
              encontrou  = 1;
           }
	   //Tentative de log por usuário cadastrado, mas senha incorreta
